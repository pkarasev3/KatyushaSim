...
rocket_1  = drawRocket03( rocket_1 ); 
rocket_2  = drawRocket03( rocket_2 );
rocket_3  = drawRocket03( rocket_3 );
plane_1   = drawPlane3D( plane_1 );
stars_1   = drawStars3D02( stars_1 );

 % send all objects to varyadic function to get image 
 vrcl      = drawMultiObject(vrcl,rocket_1,rocket_2,rocket_3,plane_1,stars_1);
 vrcl.img  = vrcl.img .*(1 + 3e-1 * randn(size(vrcl.img)) .* (1e-1 + vrcl.img) ) + 2e-2 * randn(size(vrcl.img)) ./ (1e-1 + vrcl.img);
